...
P = SX.sym('P',n_states + pl.nxo +pl.nu+(n_states^2));
...
g=[g;g_in];
obj_term_terminal= (st(1:3)-P(n_states+1:n_states+n_states)); % Terminal Constraint
QE=reshape(P((n_states + pl.nxo +pl.nu+1):end),n_states,n_states);
obj= obj+obj_term_terminal'*QE*obj_term_terminal;

% Add constraints for collision avoidance
for i=1:sim.obs_num
    for k = 1:pl.N+1
        g = [g ; -sqrt((X(1,k)-sim.obs_x(i))^2+(X(2,k)-sim.obs_y(i))^2) + pl.ego_safety_diam/2 + sim.obs_diam/2];
    end
end
...
pl_args.ubg(n_states*(pl.N+1) + n_states*pl.N+1 : n_states*(pl.N+1) + n_states*pl.N+ sim.obs_num*(pl.N+1)) = -0.05; % inequality constraints
