function sim_viz(pl,sim,viz,fbk,pl_rec,title_string) 
x_r_1 = [];
y_r_1 = [];
...
for k = 1:length(fbk)
    ...
    subplot(1,2,1)
    p1=plot(viz.map_x,viz.map_y,'-k','LineWidth',3);
    hold on
    fill(x_f_rect,y_f_rect,'k','HandleVisibility','off');
    hold on
    p2=fill(x_f_tri,y_f_tri,'m','HandleVisibility','off');
    hold on

    
    for i=1:sim.obs_num
        p3=fill(sim.obs_x(i)+viz.obs_circ_x, sim.obs_y(i)+viz.obs_circ_y,'r','HandleVisibility','off');
        hold on
    end
    ...
    hold on
    p4=plot(x01+viz.rob_circ_x,y01+viz.rob_circ_y,'--r','LineWidth',3);
    hold on
    r1.FaceColor='#7E2F8E';
    r1.EdgeColor='#7E2F8E';
    r2.FaceColor='#F56600';
    ...
    p5=plot(x_r_1,y_r_1,'Color','#7E2F8E','linewidth',3);hold on % plot exhibited trajectory
    hold on
    if k < length(fbk) % plot prediction
        p6=plot( pl_rec(1:pl.N,1,k), pl_rec(1:pl.N,2,k),'--*','Color','#F56600');
    end  
    hold off
    %figure(500)
    ...
    if k<length(fbk)
        legend([p1,p2,p3,p4,p5,p6],'Map Limits','Reference Pose','Obstacle',...
            'Ego Safety Radius','Actual Path','Predicted Path','FontSize',10, ...
            'Location','northwest')
    end
